...
class MathTest {
  ...
  void clamp_double() {
    ...
    assertThat(Math.clamp(-12d, 0d, 100d)).as("Clamped int").isZero();
    assertThat(Math.clamp(124d, 0d, 100d)).as("Clamped int").isEqualTo(100);
    assertThat(Math.clamp(0d, 0d, 100d)).as("Clamped int").isZero();
  ...
  void clamp_float() {
    ...
    assertThat(Math.clamp(0.0, 0.0, 100.0)).as("Clamped int").isZero();
